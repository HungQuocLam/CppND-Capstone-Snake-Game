...
Game::Game(std::size_t grid_width, std::size_t grid_height, int bomb_num)
    : snake(std::shared_ptr<Snake>(new Snake(grid_width, grid_height, static_cast<float>(grid_width/2), static_cast<float>(grid_height/2), Snake::Direction::kUp, Snake::Player::kPlayerOne))),
      snake2(std::shared_ptr<Snake>(new Snake(grid_width, grid_height, static_cast<float>(grid_width/2), static_cast<float>(grid_height/2), Snake::Direction::kDown, Snake::Player::kPlayerTwo))),      engine(dev()),
      random_w(0, static_cast<int>(grid_width)-1),
      random_h(0, static_cast<int>(grid_height)-1),
      winner(Snake::Player::kNULL),
      bomb_m(std::shared_ptr<Bomb_matrix>(new Bomb_matrix(static_cast<int>(grid_width), static_cast<int>(grid_height), bomb_num))),
      food(std::shared_ptr<SDL_Point>(new SDL_Point())),
      timer(std::shared_ptr<Elapse_timer>(new Elapse_timer)),
      timer2(std::shared_ptr<Elapse_timer>(new Elapse_timer)) 
{
  PlaceFood();
}
